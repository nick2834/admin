suber.filter('dataType',function(){
    ...
    .directive('searchCon',function(tools){
        return{
            restrict: 'E',
            //replace: 'true',
            templateUrl:'template/search.html?'+ new Date(),
            scope:false,
    ...
    .directive('pageCon',function($timeout,$compile,tools){
        return{
            restrict: 'E',
            replace: 'true',
            ...
            link:function(scope,element,attris){
                ...
                scope.jumpPage = function(){
                    ...
                    if(!pageNum){
                        tools.popShow("请输入跳转页面");
                        return;
                    }
                    if(pageNum <= 0){
                        tools.popShow("页码不能小于1");
                        return;
                    }
                    if(pageNum > scope.topicPage.total_pages){
                        tools.popShow("超出页码范围");
                        return;
                    }
    ...
    .directive('pageCon2',function($timeout,$compile,tools){
        return{
            restrict: 'E',
            replace: 'true',
                ...
                scope.$on('topicPage2',function(event,msg){
                    ...
                    for(var i=firstPage;i<=lastPage;i++){
                        ...
                        if(i==lastPage && msg + 5 < scope.topicPage2.total_pages){
                            $(".pagination2 li#finally").before('<li class="more">……</li>');
                            return;
                        }
                    }
                });
                scope.changePage2 = function(num){
                    if(typeof num == "number"){
                        ...
                        if(num == "pre"){
                            ...
                            scope.goPage2(scope.topicPage.current_page-1);
                        }else if(num == "next"){
                            if(scope.topicPage2.current_page==scope.topicPage2.total_pages){
                                return;
                            }
                            scope.goPage2(scope.scope.topicPage.current_page+1);
                        }
                    }
                };
